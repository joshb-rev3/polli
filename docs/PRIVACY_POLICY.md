...
> Bracketed items like `[LEGAL ENTITY NAME]` must be filled in before this can
> be used.
...
[LEGAL ENTITY NAME] ("**polli**," "**we**," "**us**," or "**our**") operates
the polli mobile application and related services (the "**Service**"). This
...
described in this Privacy Policy. If you do not agree, do not use the
Service.
...
- Authentication information via third-party sign-in (Apple, Google, Facebook)
  — we receive a token and basic profile fields from these providers; we do
  not receive your password for these services
...
- Device and usage information (device type, OS version, app version, crash
  logs, general usage analytics)
- Approximate location derived from IP address, if applicable to fraud
  prevention

### 1.4 Information from third-party services

- **Stripe:** payment status, whether identity/bank verification
  ("KYC") has been completed, and account status for payout purposes. We do
  not receive or store full card numbers, bank account numbers, or
  government ID details — those are collected and stored directly by Stripe.
- **Supabase:** our backend/database and authentication provider, which
  processes data on our behalf under its own data processing terms.
...
- Comply with legal obligations, including tax reporting and unclaimed
  property law (see the Terms of Service, Section on Unclaimed Balances)
...
- **In a business transfer** (merger, acquisition, or asset sale), subject to
  standard confidentiality protections

We do not share full payment card numbers, bank account numbers, or
government identification with anyone other than Stripe, our payment
processor.
...
Service and for legitimate business or legal purposes, including:
...
category and jurisdiction, including any state-specific requirements.]
...
- Request deletion of your information, subject to legal retention
  requirements (e.g., financial/tax records, unclaimed-property obligations)
...
[NOTE TO COUNSEL: please confirm whether CCPA/CPRA (California), and any
other state privacy statutes apply based on where the company operates and
where users are located, and expand this section accordingly.]
...
material changes as required by law and update the "Last updated" date
above.
